...
public class FCMService extends FirebaseMessagingService implements PushConstants {
    ...
    private static ArrayList<String> receivedCalls = new ArrayList<>();
    ...
    private class MyRunnable implements  Runnable{
        ...
        public MyRunnable(NotificationManager m, int id, String t){
        ...
        public void run() {
            new CountDownTimer(30000, 10000) {
                ...
                public void onFinish() {
                    ...
                    mNotificationManager.cancel(tag, notiid);
                }
            }.start();
        }
    }
    ...
    public void onMessageReceived(RemoteMessage message) {
        ...
        String notid = "", dateStr = "";
        JsonObject action = new JsonObject();

        for (Map.Entry<String, String> entry : message.getData().entrySet()) {
            Log.d(LOG_TAG, "MESSAGE DATA - " + entry.getKey() + " : " + entry.getValue());
            extras.putString(entry.getKey(), entry.getValue());
            if (entry.getKey().equals("notId")) {
                Log.d(LOG_TAG, "NOTI ID" + " : " + entry.getValue());
                notid = entry.getValue();
            }

            if (entry.getKey().equals("actionData")) {
                String actionStr = entry.getValue();
                try {
                    Gson gson = new Gson();
                    JsonElement element = gson.fromJson (actionStr, JsonElement.class);
                    action = element.getAsJsonObject();
                }catch(Exception ex){
                    Log.e(LOG_TAG,"Error while getting action data to check video call push", ex);
                }
            }
        ...
        try{
            if (action.has("type") && action.get("type").getAsString().equals("SH_VIDEO_CALL")) {
                if (receivedCalls.contains(notid)) {
                    return;
                } else {
                    receivedCalls.add(notid);
                }

                if (dateStr != null && dateStr.length() > 0) {
                    try {
                        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
                        Date pushSendDate = dateFormat.parse(dateStr);
                        TimeZone timeZone = Calendar.getInstance().getTimeZone();
                        Date dateNow = new Date();
                        int offsetMilli = timeZone.getRawOffset();
                        if (timeZone.inDaylightTime(dateNow)) {
                            offsetMilli += timeZone.getDSTSavings();
                        }

                        Log.d(LOG_TAG, "MESSAGE DATA - Local Push Send Date : " + pushSendDate.toString() + " AND LocalDateTime NOW : " + dateNow.toString() + " AND DURATION : " + (dateNow.getTime() - (pushSendDate.getTime() + offsetMilli)));
                        if (dateNow.getTime() - (pushSendDate.getTime() + offsetMilli) > 30000) {
                            return;
                        }
                    } catch (Exception pe) {
                        Log.e(LOG_TAG, "Date Parsing error. " + dateStr + " | Error:" + pe.toString());
                    }
                }
            }

            if (action.has("type") && action.get("type").getAsString().equals("SH_CANCEL_CALL")) {
    ...
    public void createNotification(Context context, Bundle extras) {
        ...
        //GET ACTION OBJECT FROM EXTRA
        String actionType = "";
        if(extras.containsKey("actionData")){
            String actionStr = extras.getString("actionData");
            try {
                Gson gson = new Gson();
                JsonElement element = gson.fromJson (actionStr, JsonElement.class);
                JsonObject action = element.getAsJsonObject();
                actionType = action.get("type").getAsString();
            }catch(Exception ex){
                Log.e(LOG_TAG,"Error while getting action data to check video call push", ex);
            }
        }
        ...
        if (actionType.equals("SH_VIDEO_CALL")) {
            mBuilder
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            mBuilder.setTimeoutAfter(30000);
        }else{
            MyRunnable myr = new MyRunnable(mNotificationManager,notId, appName);
            new Handler(Looper.getMainLooper()).post(myr);
        }
